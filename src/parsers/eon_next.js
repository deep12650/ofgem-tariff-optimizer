// E.ON Next style: "Date & Time From","Date & Time To","kWh" or "Energy (kWh)"
 ...
 */
export function parseEONNextCSV(csv, opts={}) {
  ...
  const matched = headerMatches(headerLower, {
    start: [/date.*time.*from|from/i],
    end: [/date.*time.*to|to/i],
    kwh: [/kwh|energy.*kwh/i],
  });
